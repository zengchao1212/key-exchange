...
public class WalletClientApplication {
    ...
    public static void main(String[] argv) throws IOException {
        ...
        if (connect(host, port)) {
            log.info("连接成功");
            Message.write(MessageType.CLIENT_TYPE, new byte[]{(byte) ParticipatorInfo.Type.WALLET.ordinal()}, socketChannel);
        } else {
            log.error("无法连接服务器");
            return;
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(WalletClientApplication::shutdown));
    ...
    private static void readClientMidKey(Message message, SelectionKey key) throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException, IOException {
        ...
        if (allMidKey.size() == participatorCount) {
